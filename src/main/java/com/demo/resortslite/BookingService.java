...
public class BookingService {
    ...
    // FIXED cr-java-0069: Load credentials from AWS Secrets Manager
    ...
    private void loadDatabaseCredentials() {
        try {
            ...
            this.dbHost = secretJson.has("host") ? secretJson.get("host").asText() : "localhost";
            this.dbUser = secretJson.has("username") ? secretJson.get("username").asText() : "sa";
            this.dbPassword = secretJson.has("password") ? secretJson.get("password").asText() : "";

        } catch (Exception e) {
            // Fallback to environment variables if Secrets Manager is not available
            System.err.println("Warning: Could not load credentials from AWS Secrets Manager: " + e.getMessage());
